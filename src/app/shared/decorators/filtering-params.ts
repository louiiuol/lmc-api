		...
		(target, key, index) => {
			...
			Reflect.defineMetadata(
				...
				[
					...
					{
						description:
							"Filtre(s) à appliquer. Ils peuvent être cumulés (alors séparés par '&'). Chaque filtre est formaté de la façon suivante:  property:rule:value.",
						in: 'query',
						name: 'filter',
						example: 'email:eq:email@example.fr',
						required: false,
						type: 'string',
					},
				],
